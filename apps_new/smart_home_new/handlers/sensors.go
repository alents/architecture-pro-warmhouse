...
import (
	...
	"smarthome_new/db"
	"smarthome_new/models"
	"smarthome_new/services"
...
func (h *SensorHandler) GetSensorByID(c *gin.Context) {
	...
	// Получаем телеметрию с внешнего сервиса
	telemetry, err := h.TelemetryService.GetTelemetryBySensorID(id)
	if err != nil {
		log.Printf("Failed to fetch telemetry for sensor %d: %v", id, err)
		telemetry = []services.TelemetryResponse{}
	}
